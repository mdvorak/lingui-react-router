"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);

// src/index.ts
var index_exports = {};
__export(index_exports, {
  LocalizedRedirect: () => LocalizedRedirect,
  useLocalizedNavigate: () => useLocalizedNavigate,
  useRouteLocale: () => useRouteLocale
});
module.exports = __toCommonJS(index_exports);

// src/components.tsx
var import_react_router_dom = require("react-router-dom");
var import_react = require("@lingui/react");
var import_jsx_runtime = require("react/jsx-runtime");
function LocalizedRedirect({ to, replace = false, state }) {
  const params = (0, import_react_router_dom.useParams)();
  const { i18n } = (0, import_react.useLingui)();
  const locale = params.locale || i18n.locale;
  const localizedPath = `/${locale}${to.startsWith("/") ? to : `/${to}`}`;
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_react_router_dom.Navigate, { to: localizedPath, replace, state });
}

// src/hooks.ts
var import_react_router_dom2 = require("react-router-dom");
var import_react2 = require("@lingui/react");
function useLocalizedNavigate() {
  const navigate = (0, import_react_router_dom2.useNavigate)();
  const { i18n } = (0, import_react2.useLingui)();
  const params = (0, import_react_router_dom2.useParams)();
  const localizedNavigate = (to, options = {}) => {
    const locale = options.locale || params.locale || i18n.locale;
    const localizedPath = `/${locale}${to.startsWith("/") ? to : `/${to}`}`;
    navigate(localizedPath, {
      replace: options.replace,
      state: options.state
    });
  };
  return localizedNavigate;
}
function useRouteLocale() {
  const params = (0, import_react_router_dom2.useParams)();
  const { i18n } = (0, import_react2.useLingui)();
  return params.locale || i18n.locale;
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  LocalizedRedirect,
  useLocalizedNavigate,
  useRouteLocale
});
//# sourceMappingURL=index.cjs.map